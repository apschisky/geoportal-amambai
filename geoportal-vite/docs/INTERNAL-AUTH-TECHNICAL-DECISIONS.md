...
Atualizacao de validacao manual autenticada via proxy HTTPS real: apos a configuracao controlada de `/api/internal/` no Apache e do proxy Vite apenas para desenvolvimento, o login interno foi testado manualmente contra `https://geoserver.amambai.ms.gov.br/api/internal/auth/login`. O resultado sanitizado foi `LOGIN_STATUS=200`. Em seguida, na mesma sessao PowerShell, `GET /api/internal/auth/me` retornou `ME_STATUS=200`, `AUTHENTICATED=True`, `USUARIO_ID=7`, `PERMISSOES_COUNT=15` e `TEM_ILUMINACAO_LER=True`. Essa validacao confirma que a sessao/cookie criado pelo login foi aceito pelo `/me`, que o usuario validado possui a permissao base `iluminacao.solicitacoes.ler` e que a liberacao do modulo Iluminacao por permissao esta tecnicamente pronta em homologacao. A documentacao nao registra senha, token, cookie real, hash, `session_secret` ou `DATABASE_URL`; o token retornado no corpo do login deve continuar sendo ignorado pela shell e nunca armazenado no frontend.

Decisao tecnica de UX/autenticacao: o login visual inicial deve ser implementado dentro da shell interna `/interno/`. O Geoportal publico pode futuramente ter apenas um link para `/interno/`, mas nao deve acoplar popup de login ao mapa publico como primeira implementacao. A shell deve executar `GET /api/internal/auth/me`, exibir formulario interno somente quando houver `401`, chamar `POST /api/internal/auth/login`, ignorar o token do corpo, depender do cookie HttpOnly, repetir `/me` e montar menu por permissoes efetivas. O Geoportal publico nao deve manipular token, cookie, senha ou estado de sessao interna.
...
## Decisão sobre Reset Administrativo de Senha de Usuário Interno

- Endpoint preferido (documental): `POST /api/internal/admin/users/{usuario_id}/reset-password` para contrato explícito.
- Payload: `nova_senha` (obrigatório) e `confirmar_nova_senha` (obrigatório); sem aceitar senha por query string ou path.
- Validações: senhas idênticas e seguramente comparadas; `nova_senha` cumprindo política centralizada (6-128 chars, letra+número, não igual a login/nome, bloqueada se comum); rejeição de campos extras.
- Persistência: gerar novo hash Argon2id; atualizar `mod_auth.usuarios.senha_hash` e `mod_auth.usuarios.atualizado_em` (se aplicável); revogar sessões ativas atualizando `mod_auth.sessoes.revogado_em = now()` (sem DELETE físico).
- Efeitos: senha antiga deixa de funcionar; sessões ativas revogadas imediatamente; usuário bloqueado continua bloqueado (sem desbloquear automaticamente); não altera perfil, permissões ou `ativo`.
- Proteções: exigir `GEOPORTAL_INTERNAL_ROUTES_ENABLED`, sessão autenticada, `require_permission("admin.usuarios.redefinir_senha")` e header `X-Geoportal-Internal-Request: 1` para rotas mutáveis.
- Respostas: 200 com usuário sanitizado (`id`, `login`, `nome`, `email`, `ativo`, `bloqueado`, `criado_em`), sem retornar `senha`, `nova_senha`, `senha_hash`, `bloqueado_ate`, `atualizado_em`, `ultimo_login_em`, token, `token_hash`, cookie, `session_secret`, `DATABASE_URL`, SQL, role, GRANT, sessão ou auditoria.
- Observação: esta decisão é documental — não aplicar migrations, roles, grants, código ou alterações em produção nesta etapa.

### Planejamento operacional em homologação

Reset administrativo de senha será validado em homologação com `teste.criacao` (`id=8`) após implementação com Codex High. Cenários planejados: 401 sem sessão; 403 sem header mutável; 422 para senha fraca/inválida; 422 se senhas divergem; 200 reset bem-sucedido; confirmar que login com senha antiga falha (401); confirmar que login com nova senha funciona (200); confirmar que sessões ativas são revogadas (`revogado_em` preenchido, acesso negado 401); confirmar que usuário bloqueado continua bloqueado após reset (sem desbloquear). Resposta permanecerá sanitizada; nenhum dado sensível exposto. Variáveis temporárias serão limpas. Produção, NSSM, `.env` versionado e frontend não serão alterados.

### Implementacao backend

O endpoint `POST /api/internal/admin/users/{usuario_id}/reset-password` foi implementado como acao mutavel administrativa sensivel. Ele exige `GEOPORTAL_INTERNAL_ROUTES_ENABLED`, sessao autenticada, `require_permission("admin.usuarios.redefinir_senha")` e `X-Geoportal-Internal-Request: 1`. O payload aceita somente `nova_senha` e `confirmar_nova_senha`, rejeita campos extras e converte divergencia/senha fraca em 422 generico.

O fluxo reutiliza a politica centralizada de senha antes de `hash_password(...)`, gera hash Argon2id, atualiza apenas `mod_auth.usuarios.senha_hash` e `mod_auth.usuarios.atualizado_em`, e revoga sessoes ativas com `mod_auth.sessoes.revogado_em = now()`, sem `DELETE` fisico. Reset nao desbloqueia usuario, nao altera `ativo`, perfis, permissoes ou vinculos, nao cria sessao e nao escreve auditoria de login nesta etapa.
...
Decisao tecnica implementada para bloqueio/desbloqueio: `POST /api/internal/admin/users/{usuario_id}/block` e `POST /api/internal/admin/users/{usuario_id}/unblock` foram criados como endpoints mutaveis explicitos, sem usar `PATCH` generico. Ambos exigem sessao autenticada, `require_permission("admin.usuarios.bloquear")`, `X-Geoportal-Internal-Request: 1` e feature flag interna. O bloqueio persiste em `mod_auth.usuarios.bloqueado_ate` com timestamp futuro e revoga sessoes ativas do usuario com `mod_auth.sessoes.revogado_em = now()`, sem `DELETE`. O desbloqueio limpa `bloqueado_ate` e nao cria sessao. A resposta expõe apenas `bloqueado` booleano derivado dentro do envelope `usuario`, nunca `bloqueado_ate`. A implementacao nao altera senha, perfis, permissoes ou auditoria, nao envia e-mail, nao cria migration/schema, nao altera producao, NSSM, `.env`, frontend ou tela. A etapa posterior de reset administrativo foi implementada separadamente.
...
Atualizacao do commit `9173259`: a remocao logica de vinculos usuario/perfil deixou de ser apenas lacuna planejada e foi implementada no backend, sem frontend e sem migration estrutural. A validacao operacional foi concluida em homologacao e producao interna em 2026-06-26.
...
A decisao operacional aplicada foi: homologacao primeiro, bootstrap controlado de `admin.usuarios.remover_perfis`, GRANT minimo de `UPDATE (ativo)` em `mod_auth.usuario_perfis`, sem `DELETE`, e producao somente em ciclo separado apos validacao.
...
## Validacao tecnica em producao interna - desativacao de vinculos

Em 2026-06-26, a implementacao `9173259` foi validada em `InternaProducao`, no servico `GeoportalAPIInternaProducao`, porta `8003`, banco `amambaiGis`. A decisao tecnica de usar desativacao logica (`ativo=false`) em `mod_auth.usuario_perfis`, sem `DELETE`, foi confirmada em ambiente real de producao interna.

A permissao `admin.usuarios.remover_perfis` foi criada como permissao de aplicacao (`modulo='admin'`, `chave='usuarios.remover_perfis'`, id `19`) e associada apenas ao perfil `administrador-interno-geoportal`. O perfil `manutencao-iluminacao` nao recebeu essa permissao.

A matriz final de `geoportal_api_interna_prod` confirmou a separacao de capacidades: `INSERT` em `mod_auth.usuario_perfis` permanece por causa do endpoint de atribuicao ja existente, enquanto a desativacao usa apenas `UPDATE(ativo)`. Nao ha table `UPDATE` amplo nem `DELETE`. `mod_auth.permissoes` e `mod_auth.perfil_permissoes` ficaram sem `INSERT`/`UPDATE` para a role runtime apos revogacao dos GRANTs temporarios.

A resposta externa de auto-rebaixamento permaneceu `403` sanitizado, enquanto o motivo tecnico `self_demotion` ficou apenas na auditoria administrativa. O teste positivo controlado retornou `ativo=false`, a repeticao retornou `409`, e o usuario real `manutencao.producao` nao foi alterado.
