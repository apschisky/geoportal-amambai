...
Regra visual atual da manutencao: chamados em status terminal somem da listagem de campo. A comparacao normaliza status tecnico e status formatado, cobrindo `resolvida`/`Resolvida`, `cancelada`/`Cancelada`, `indeferida`/`Indeferida` e `nao_localizado`/`Não localizado`. Essa regra evita que a equipe de manutencao acompanhe chamados que nao exigem mais acao. Administrador/perfil completo continua vendo todos os chamados para auditoria, conferencia e futura volta de fase controlada. Como a regra atual ainda e refinamento de frontend, a evolucao correta para escala e criar filtro backend de chamados ativos.
...
A shell continua sem dashboard real, anexos ou correcao/reversao administrativa. O mapa atual e propositalmente simples e fica no detalhe; mapa operacional amplo e dashboard continuam fases futuras. A proxima subfase recomendada e consolidar piloto controlado, monitorar operacao real, validar a listagem compacta de manutencao em uso de campo, planejar filtro backend de chamados ativos para manutencao e tratar correcao/reabertura administrativa em contrato separado.
...
3. **Contrato backend para listagem ativa da manutencao.** Planejar filtro como `GET /api/internal/iluminacao/solicitacoes?ativos=true` ou equivalente, para que status terminais sejam excluidos pelo backend antes da paginacao. `ativos=true` deve retornar somente chamados nao terminais; ausencia do filtro ou `ativos=false` deve preservar listagem completa para perfis autorizados.
4. **Relatorio administrativo sanitizado.** Planejar endpoint backend para relatorio/exportacao, restrito a perfil administrativo/autorizado, com filtros por periodo, status, prioridade e tipo. A exportacao nao deve ser apenas a tabela visivel no frontend, pois a tabela pode estar paginada, filtrada visualmente ou incompleta.
