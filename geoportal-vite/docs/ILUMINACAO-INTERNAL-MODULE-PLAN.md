...
O frontend interno ja inclui a UI administrativa restrita para correcao de status no detalhe da solicitacao, com bloco separado `Ações administrativas`, visibilidade condicionada a `iluminacao.solicitacoes.corrigir_status`, validacao client-side de justificativa e ciencia, e chamada ao endpoint `PATCH /api/internal/iluminacao/solicitacoes/{id}/status-correcao`. A implementacao foi registrada no commit `e18fb8d Adiciona UI administrativa para correcao de status` e publicada/validada em producao interna. O mapa atual continua como visualizacao simples no detalhe da solicitacao; dashboard, anexos e mapa operacional amplo permanecem etapas separadas. A UX de manutencao em campo continua refinada com listagem compacta, rota proxima ao protocolo e alteracao rapida de fase/status conforme permissao.

Correcao administrativa implementada localmente: a volta de fase, correcao de status e reabertura de chamado terminal entraram no backend local como fluxo administrativo separado em `PATCH /api/internal/iluminacao/solicitacoes/{id}/status-correcao`. A permissao efetiva e `iluminacao.solicitacoes.corrigir_status`, representada no banco como `modulo = 'iluminacao'` e `chave = 'solicitacoes.corrigir_status'`, restrita a perfil administrativo/autorizado e fora do perfil `manutencao-iluminacao`. O payload v1 aceita apenas `novo_status` e `justificativa`; o backend infere a natureza da operacao pelo status atual e pelo novo status. Ao reabrir terminal para status ativo, a implementacao local limpa `finalizado_em` para `NULL`, mantendo o encerramento anterior no historico. Esse fluxo nao altera prioridade, protocolo, geometria, dados pessoais ou observacoes internas e nao foi implementado pelo PATCH normal da manutencao.

UI administrativa registrada em producao interna: a validacao visual/operacional foi feita em `https://geoserver.amambai.ms.gov.br/interno/` com `admin.producao` e `manutencao.producao`. O usuario administrativo viu o painel `Ações administrativas`, a acao ficou disponivel, as validacoes client-side funcionaram, o endpoint real foi executado com sucesso e detalhe/historico foram recarregados. O usuario de manutencao nao viu a acao, mantendo a separacao visual e operacional prevista. O contrato detalhado segue em `docs/API-ENDPOINTS-ILUMINACAO.md`.

Dashboard gerencial read-only implementado localmente no backend: os endpoints `GET /api/internal/iluminacao/dashboard/resumo`, `GET /api/internal/iluminacao/dashboard/ranking` e `GET /api/internal/iluminacao/dashboard/series` foram adicionados como leitura agregada e sanitizada, protegidos por `iluminacao.dashboard.ler`. Eles nao retornam nome do solicitante, contato/WhatsApp, descricao livre, observacoes internas, historico administrativo ou coordenadas. O ranking de postes usa `poste_id`; `top_bairros` fica vazio na v1 porque o schema real ainda nao possui campo de bairro/regiao. Esta etapa nao implementa frontend, nao cria migration e nao executa SQL. Antes de deploy, a permissao `modulo = 'iluminacao'`, `chave = 'dashboard.ler'` deve ser criada/concedida por procedimento operacional controlado ao perfil gerencial/administrativo adequado, sem conceder ao perfil `manutencao-iluminacao`.

Inventario consolidado com homologacao e validacao em producao interna: as migrations versionadas e as consultas somente leitura em homologacao confirmam que `mod_iluminacao.solicitacoes` ja permite todos os status atuais e possui `finalizado_em` anulavel; `mod_iluminacao.solicitacoes_historico` ja possui campos para status anterior/novo, usuario, origem administrativa e justificativa resumida. A constraint de `acao` aceita `reabertura`, mas nao ha valor dedicado `correcao_status` confirmado. Portanto, a v1 validada dispensou migration estrutural, usando `reabertura` para terminal -> ativo e `alteracao_status` com `origem_acao='ajuste_administrativo'` nas demais correcoes. Se a revisao futura exigir acao dedicada `correcao_status`, a migration de constraint deve ser planejada em ciclo separado. A permissao foi criada/vinculada em producao interna por procedimento operacional controlado, com backup previo; a role runtime falhou ao tentar escrever em `mod_auth.permissoes`, comportamento coerente com menor privilegio. O perfil `administrador-interno-geoportal` recebeu a permissao e o perfil `manutencao-iluminacao` nao recebeu.
...
Matriz atual de alteracao normal de status: o backend preserva todos os status existentes e passou a permitir `aberta -> em_execucao` no fluxo normal, alem de manter `aberta -> em_triagem`, `aberta -> cancelada` e `aberta -> indeferida`. Isso atende a operacao enxuta em que a mesma pessoa pode receber e executar diretamente o chamado. `em_triagem` continua existindo e opcional; `encaminhada` continua existindo para fluxos com repasse entre equipes. As transicoes terminais permanecem conservadoras e volta de fase/reabertura continua fora deste ciclo.

Validacao publicada do atalho operacional: a transicao `aberta -> em_execucao` ja foi publicada e validada em chamado teste/controlado. O evento continuou registrando historico normalmente, o chamado permaneceu ativo para a equipe de manutencao e `finalizado_em` seguiu nulo, confirmando que a regra nao transforma a execucao direta em encerramento.

Contrato atual de identificacao visual: `/api/internal/auth/me` ja retorna `authenticated`, `usuario_id`, `login`, `nome`, `perfis` e `permissoes`, sem expor token, cookie, hash ou outros dados sensiveis. A shell usa `nome` como preferencia visual, pode mostrar `login` como complemento e preserva o fallback `Usuario interno #<id>` apenas quando a resposta vier incompleta.
...
A shell interna ja passou a incluir o Dashboard geral implementado no frontend `/interno/` para usuarios com `iluminacao.dashboard.ler`, com KPIs compactos, ranking, serie semanal, filtros e mapa gerencial v1 baseado na listagem operacional autorizada. O modulo operacional de Iluminacao Publica permanece preservado; usuarios sem a permissao de dashboard continuam no fluxo operacional e nao recebem os widgets gerenciais do dashboard. Anexos e a tela de correcao/reversao administrativa continuam como etapas separadas, e o mapa operacional amplo segue sendo uma evolucao posterior.
7. Fase 7: correcao administrativa de status validada na API interna de producao. O servidor foi atualizado ate `da4be5c`, `GeoportalAPIInternaProducao` foi reiniciado/validado pelo harness `InternaProducao`, a permissao `modulo = 'iluminacao'` + `chave = 'solicitacoes.corrigir_status'` foi criada/vinculada ao perfil administrativo com backup previo e o perfil `manutencao-iluminacao` permaneceu sem a permissao. A validacao direta autenticada reabriu o chamado teste/controlado `IP-2026-000001` de `resolvida` para `em_execucao`, limpou `finalizado_em`, preservou `prioridade=normal` e gravou historico `acao='reabertura'`, `origem_acao='ajuste_administrativo'`. Depois, a validacao negativa zero-write confirmou 403 sem header, 422 para campo extra/justificativa curta/status inexistente, 404 para solicitacao inexistente e 403 para `manutencao.producao` sem permissao, sem alterar `status`, `atualizado_em` ou `finalizado_em` do chamado teste. Pendencias: revisar o estado operacional desse chamado teste, decidir se os cenarios `terminal -> aberta` e `terminal -> encaminhada` exigem validacao manual adicional e implementar frontend administrativo restrito com modal, justificativa obrigatoria e testes.
8. Fase 8: auditoria e revisao de seguranca antes de uso por equipe real.
9. Fase 9: mapa interno amplo, relatorios evoluidos e indicadores.
...
6. **Dashboard real.** O commit `ebc5798` registrou a implantacao dos endpoints read-only `GET /api/internal/iluminacao/dashboard/resumo`, `GET /api/internal/iluminacao/dashboard/ranking` e `GET /api/internal/iluminacao/dashboard/series` em producao interna. Eles permanecem protegidos por `iluminacao.dashboard.ler`, retornam apenas agregados sanitizados e nao devem ser confundidos com frontend do Dashboard nem com o mapa operacional amplo. A validacao em servidor incluiu `git pull --ff-only`, `HEAD` em `ebc5798`, working tree clean, testes focados `197 passed, 3 warnings`, suite completa `675 passed, 3 warnings`, restart do runtime `GeoportalAPIInternaProducao` via harness, e checagens HTTP com `401` sem sessao, `200` para `admin.producao`, `403` para `manutencao.producao` e `422` para parametro invalido de series.
7. **Mapa operacional interno.** O mapa deve permanecer como visao territorial separada do Dashboard, com agrupamento de pontos, filtros por status/periodo/regiao e acesso restrito ao detalhe. Nao deve expor dados pessoais, observacoes internas, contato completo, geometria sensivel ou historico administrativo fora do ambiente interno.
8. **Correcao/reabertura administrativa.** Backend implementado e validado na API interna de producao como fluxo separado do PATCH normal de status em `status-correcao`, com permissao propria `iluminacao.solicitacoes.corrigir_status`, justificativa forte, auditoria explicita, sem migration estrutural e bloqueio contra uso pela manutencao. A validacao negativa zero-write confirmou erros 403/422/404 esperados sem mutar o chamado teste. Pendencias: revisar o chamado teste/controlado deixado em `em_execucao`, decidir sobre validacao manual adicional de `terminal -> aberta/encaminhada` e implementar frontend administrativo no detalhe da solicitacao, separado do fluxo normal e oculto para manutencao.
