...
Planejamento da correcao administrativa: a volta de fase, correcao de status e reabertura de chamado terminal devem entrar como fluxo administrativo separado, planejado como `PATCH /api/internal/iluminacao/solicitacoes/{id}/status-correcao`. A permissao efetiva proposta e `iluminacao.solicitacoes.corrigir_status`, representada no banco como `modulo = 'iluminacao'` e `chave = 'solicitacoes.corrigir_status'`, restrita a perfil administrativo/autorizado e fora do perfil `manutencao-iluminacao`. O payload planejado aceita apenas `novo_status` e `justificativa`; o backend deve inferir se a operacao e reabertura, volta de fase ou correcao de status. Ao reabrir terminal para status ativo, a recomendacao e limpar `finalizado_em` para `NULL`, mantendo o encerramento anterior no historico. Esse fluxo nao altera prioridade, protocolo, geometria, dados pessoais ou observacoes internas e nao deve ser implementado pelo PATCH normal da manutencao.

Inventario consolidado com homologacao: as migrations versionadas e as consultas somente leitura em homologacao confirmam que `mod_iluminacao.solicitacoes` ja permite todos os status atuais e possui `finalizado_em` anulavel; `mod_iluminacao.solicitacoes_historico` ja possui campos para status anterior/novo, usuario, origem administrativa e justificativa resumida. A constraint de `acao` aceita `reabertura`, mas nao ha valor dedicado `correcao_status` confirmado. Portanto, a v1 pode dispensar migration estrutural se usar `reabertura` para terminal -> ativo e `alteracao_status` com `origem_acao='ajuste_administrativo'` nas demais correcoes. Se a revisao exigir acao dedicada `correcao_status`, a migration de constraint deve ser planejada antes do backend. A permissao nova ainda nao existe em homologacao e precisa de seed/script administrativo controlado; nenhum perfil a possui hoje e ela nao deve entrar no perfil `manutencao-iluminacao`.
...
7. Fase 7: implementar correcao administrativa de status em subfases, somente apos revisao humana do contrato: usar a confirmacao real de homologacao como base, decidir se a v1 fica com os valores ja aceitos (`reabertura` e `alteracao_status` + `origem_acao='ajuste_administrativo'`) ou se precisara de migration para `correcao_status`, criar/vincular a permissao `modulo = 'iluminacao'` + `chave = 'solicitacoes.corrigir_status'` por seed/script controlado, reaproveitar os GRANTs por coluna ja existentes em `status`, `atualizado_em` e `finalizado_em`, validar com chamado teste/controlado e entao expor acao administrativa no frontend. Manutencao continua sem essa permissao.
...
7. **Correcao/reabertura administrativa.** Implementar somente como fluxo separado do PATCH normal de status, planejado como `status-correcao`, com permissao propria `iluminacao.solicitacoes.corrigir_status`, justificativa forte, auditoria explicita, revisao de schema antes de qualquer migration e bloqueio contra uso acidental.
