...
7. Fase 7: implementar correcao administrativa de status em subfases, somente apos revisao humana do contrato: confirmar schema real em homologacao/producao interna com SQL somente leitura, decidir se a v1 usa valores ja aceitos (`reabertura` e `alteracao_status` + `origem_acao='ajuste_administrativo'`) ou se precisara de migration para `correcao_status`, criar/vincular a permissao `iluminacao.solicitacoes.corrigir_status` por seed/script controlado, validar GRANT minimo em homologacao, implementar backend local com testes, validar com chamado teste/controlado e entao expor acao administrativa no frontend. Manutencao continua sem essa permissao.
