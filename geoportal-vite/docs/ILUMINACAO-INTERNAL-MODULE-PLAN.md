...
6. **Dashboard real.** O plano recomendado para esta etapa e separar a leitura gerencial em duas telas internas: `Dashboard` para indicadores/metricas e `Mapa` para visualizacao territorial. A implementacao futura deve criar endpoints agregados no backend para indicadores, evitando calculos criticos no frontend. Priorizar chamados ativos, finalizados, por status, por prioridade, por tipo e por periodo. A permissao `iluminacao.dashboard.ler` deve ser avaliada como leitura consolidada especifica, sem conceder acoes operacionais.
