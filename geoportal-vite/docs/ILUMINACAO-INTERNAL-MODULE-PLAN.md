...
7. **Correcao/reabertura administrativa.** Backend implementado e validado na API interna de producao como fluxo separado do PATCH normal de status em `status-correcao`, com permissao propria `iluminacao.solicitacoes.corrigir_status`, justificativa forte, auditoria explicita, sem migration estrutural e bloqueio contra uso pela manutencao. A validacao negativa zero-write confirmou erros 403/422/404 esperados sem mutar o chamado teste. Pendencias: revisar o chamado teste/controlado deixado em `em_execucao`, decidir sobre validacao manual adicional de `terminal -> aberta/encaminhada` e planejar frontend administrativo.
