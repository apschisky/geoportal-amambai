...
O frontend interno continua sem dashboard real, anexos ou tela administrativa para correcao/reabertura de status terminal. O backend local da correcao administrativa foi implementado no commit `313afd8 Implementa correcao administrativa de status`, mas ainda depende de validacao em servidor, bootstrap real da permissao e frontend administrativo proprio antes de uso operacional. O mapa atual e uma visualizacao simples no detalhe da solicitacao; mapa operacional amplo e dashboard permanecem etapas separadas. A UX de manutencao em campo foi refinada com listagem compacta, rota proxima ao protocolo e alteracao rapida de fase/status conforme permissao.
...
Inventario consolidado com homologacao: as migrations versionadas e as consultas somente leitura em homologacao confirmam que `mod_iluminacao.solicitacoes` ja permite todos os status atuais e possui `finalizado_em` anulavel; `mod_iluminacao.solicitacoes_historico` ja possui campos para status anterior/novo, usuario, origem administrativa e justificativa resumida. A constraint de `acao` aceita `reabertura`, mas nao ha valor dedicado `correcao_status` confirmado. Portanto, a v1 local dispensou migration estrutural, usando `reabertura` para terminal -> ativo e `alteracao_status` com `origem_acao='ajuste_administrativo'` nas demais correcoes. Se a revisao futura exigir acao dedicada `correcao_status`, a migration de constraint deve ser planejada em ciclo separado. A permissao foi adicionada ao bootstrap administrativo local, mas ainda nao existe/vincula em homologacao ou producao ate execucao controlada do script; nenhum perfil real a possui nesses ambientes e ela nao deve entrar no perfil `manutencao-iluminacao`.
...
A shell continua sem dashboard real, anexos ou tela de correcao/reversao administrativa. O mapa atual e propositalmente simples e fica no detalhe; mapa operacional amplo e dashboard continuam fases futuras. A proxima subfase recomendada e consolidar piloto controlado, monitorar operacao real, validar a listagem compacta de manutencao com `ativos=true` em uso de campo, validar o relatorio administrativo sanitizado em operacao inicial e validar em servidor o backend local de correcao/reabertura administrativa.
7. Fase 7: validar e publicar gradualmente a correcao administrativa de status ja implementada localmente no backend: fazer pull do commit `313afd8` no servidor, rodar testes focados, criar/vincular a permissao `modulo = 'iluminacao'` + `chave = 'solicitacoes.corrigir_status'` por bootstrap administrativo controlado, confirmar que `manutencao-iluminacao` nao recebe a permissao, reaproveitar os GRANTs por coluna ja existentes em `status`, `atualizado_em` e `finalizado_em`, reiniciar a API interna com harness, validar por chamada direta autenticada em chamado teste/controlado e somente depois planejar frontend administrativo. Manutencao continua sem essa permissao.
...
7. **Correcao/reabertura administrativa.** Backend local implementado como fluxo separado do PATCH normal de status em `status-correcao`, com permissao propria `iluminacao.solicitacoes.corrigir_status`, justificativa forte, auditoria explicita, sem migration estrutural e bloqueio contra uso pela manutencao. Pendencia: validacao em servidor, bootstrap da permissao real, restart controlado e frontend administrativo.
