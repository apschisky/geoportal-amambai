...
O frontend interno continua sem dashboard real, anexos ou tela administrativa para correcao/reabertura de status terminal. O backend da correcao administrativa foi implementado no commit `313afd8 Implementa correcao administrativa de status` e validado na API interna de producao apos pull/restart em `GeoportalAPIInternaProducao`. Ainda falta frontend administrativo proprio antes de uso operacional amplo. O mapa atual e uma visualizacao simples no detalhe da solicitacao; mapa operacional amplo e dashboard permanecem etapas separadas. A UX de manutencao em campo foi refinada com listagem compacta, rota proxima ao protocolo e alteracao rapida de fase/status conforme permissao.
...
Planejamento da UI administrativa para `status-correcao`: a primeira tela deve ficar no detalhe da solicitacao, em bloco separado de `Acoes administrativas`, visivel somente para usuario com `iluminacao.solicitacoes.corrigir_status`. Ela nao deve aparecer para `manutencao-iluminacao` nem para `manutencao.producao`. A acao deve abrir modal de confirmacao forte, mostrar status atual e novo status, exigir justificativa trimada de 10 a 1000 caracteres, exigir ciencia explicita e chamar o endpoint com `X-Geoportal-Internal-Request: 1`. O historico deve destacar `acao='reabertura'` e `origem_acao='ajuste_administrativo'`. A primeira UI nao inclui administracao de usuarios/perfis/permissoes, dashboard avancado, edicao direta de banco, atalho para manutencao ou acao em lote. O contrato detalhado esta em `docs/API-ENDPOINTS-ILUMINACAO.md`.
