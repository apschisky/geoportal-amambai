...
Matriz atual de alteracao normal de status: o backend continua permitindo `aberta -> em_triagem`, mas nao permite `aberta -> em_execucao` no fluxo normal atual. As transicoes terminais permanecem conservadoras e qualquer atalho operacional direto de `aberta` para `em_execucao` deve ser tratado como evolucao contratual separada, com historico obrigatorio e validacao explicita de impacto para manutencao e administracao.
