...
A shell continua sem alteracao de prioridade, dashboard real, mapa operacional, anexos ou rota Google Maps. Reabertura ou correcao administrativa de status terminal ficam fora desta fase. A proxima subfase recomendada e revisar o contrato planejado de prioridade operacional antes de qualquer implementacao de backend, migration, permissao real, GRANT ou frontend.

Planejamento da prioridade operacional: a prioridade deve apoiar triagem, ordenacao operacional, destaque visual futuro, atendimento e dashboard, sem substituir o status. Valores recomendados: `baixa`, `normal`, `alta` e `urgente`, com default `normal`. A alteracao futura deve ser independente da alteracao de status, exigir permissao especifica `iluminacao.solicitacoes.atualizar_prioridade`, header `X-Geoportal-Internal-Request: 1`, justificativa obrigatoria e historico transacional com `acao='alteracao_prioridade'`. Para a primeira versao, a regra conservadora recomendada e bloquear alteracao de prioridade em status terminal pelo fluxo normal; qualquer correcao em chamado finalizado deve ser fluxo administrativo futuro.
