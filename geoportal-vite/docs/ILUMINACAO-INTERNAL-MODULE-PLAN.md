...
6. **Dashboard real.** A base backend read-only local ja foi implementada com endpoints agregados para `resumo`, `ranking` e `series`, protegidos por `iluminacao.dashboard.ler` e sem retorno de dados pessoais. O proximo passo desta frente e criar/conceder a permissao por procedimento operacional controlado antes de deploy, validar os endpoints em servidor e so depois planejar o frontend do Dashboard. A tela deve continuar separada do `Mapa` operacional amplo e nao deve calcular indicadores criticos apenas no frontend.
