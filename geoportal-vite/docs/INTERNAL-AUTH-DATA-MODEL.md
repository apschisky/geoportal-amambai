...
7. → Implementar endpoint controlado somente depois de transporte de token, middleware/dependency e testes.
8. → Implementar autorização por dependência, middleware ou mecanismo equivalente.
9. → Testar acesso autorizado e negado com auditoria registrada.
10. → Criar endpoints internos operacionais somente depois da autenticação/autorização validada.
