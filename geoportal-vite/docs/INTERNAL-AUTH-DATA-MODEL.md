...
Registro de feature flag: `geoportal-backend/app/core/internal_routes_config.py` foi criado com a variavel futura `GEOPORTAL_INTERNAL_ROUTES_ENABLED`. A politica e fail-closed: ausencia, vazio, valor invalido ou valor desligado retornam `False`; apenas `true`, `1`, `yes` e `on` ativam o parser. Nesta etapa, a flag nao inclui router no app principal e nao ativa rota interna.
