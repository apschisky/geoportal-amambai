...
Registro documental: foi criada estrutura preparatoria para futura criacao manual do primeiro usuario interno por script administrativo em `geoportal-backend/scripts/admin/create_internal_user.py`. A tabela alvo e `mod_auth.usuarios`, com `nome`, `email`, `login` e `senha_hash` obrigatorios; `ativo` tem default `true`; `criado_em` tem default `now()`; `atualizado_em`, `bloqueado_ate`, `ultimo_login_em` e `desativado_em` permanecem nulos ate uso real. `email` e `login` possuem unicidade por indice case-insensitive. O script le senha por `getpass`, nao aceita senha por CLI, nao imprime senha/hash e possui `--dry-run` sem conexao com banco. O repository administrativo usa bind parameters e recebe somente `senha_hash`. Nenhum usuario real foi criado, nenhum seed/migration foi criado e nenhum endpoint de login foi implementado.
