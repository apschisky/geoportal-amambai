...
### Contrato Implementado do PATCH Status

O schema atual suporta o `PATCH /api/internal/iluminacao/solicitacoes/{id}/status` sem nova migration. A implementacao de backend mantem auditoria obrigatoria em `mod_iluminacao.solicitacoes_historico` na mesma transacao do `UPDATE`.
