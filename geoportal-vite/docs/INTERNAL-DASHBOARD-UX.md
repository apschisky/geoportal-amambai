...
Regra atual implementada na visao de manutencao: chamados em status terminal ficam ocultos da listagem de campo para evitar confusao operacional. A comparacao normaliza tanto status tecnico quanto status ja formatado, cobrindo `resolvida`/`Resolvida`, `cancelada`/`Cancelada`, `indeferida`/`Indeferida` e `nao_localizado`/`Não localizado`. A visao administrativa/perfil completo continua exibindo todos os chamados para auditoria, conferencia e futura volta de fase controlada.

Pendencia tecnica: quando houver maior volume de chamados ou paginacao relevante, a filtragem de chamados ativos deve passar para contrato de API, por exemplo `GET /api/internal/iluminacao/solicitacoes?ativos=true` ou filtro equivalente. O comportamento esperado e `ativos=true` retornar somente status nao terminais, ausencia do filtro manter a listagem completa para perfis autorizados, e o backend aplicar a regra antes da paginacao. O filtro visual atual e apenas refinamento operacional imediato.

Relatorio administrativo futuro: a exportacao de solicitacoes/servicos deve ser implementada por endpoint backend autorizado, nao como exportacao simples da tabela renderizada no frontend. A versao inicial deve usar filtros por periodo, status, prioridade e tipo, incluir campos sanitizados como protocolo, status, prioridade, tipo, poste, origem, bairro/rua quando ja existirem no contrato, datas de abertura/atualizacao/finalizacao e indicadores por periodo, e excluir por padrao nome, telefone/WhatsApp, observacoes internas livres e descricao livre quando houver risco de dados pessoais. A exportacao deve ser administrativa, nao disponivel para manutencao.
...
- Planejar contrato backend para listagem ativa da manutencao (`ativos=true` ou equivalente), evitando depender apenas de filtro visual no frontend quando houver muitos chamados ou paginas com finalizados.
- Planejar relatorio administrativo sanitizado por endpoint backend, com filtros por periodo e indicadores, antes de qualquer exportacao baseada na tabela visivel.
