...
Para manter consistencia terminologica, este documento considera o `Dashboard geral do Geoportal Interno` como a tela consolidada e inicial do portal, enquanto os endpoints `GET /api/internal/iluminacao/dashboard/*` continuam sendo contratos especificos do modulo Iluminacao Publica para widgets e agregados read-only. O Dashboard geral ja foi implementado e publicado no frontend interno em `/interno/`, e usa esses widgets quando aplicavel, sem depender de endpoints gerais `/api/internal/dashboard/*`.

Diretriz atual da UX: o item lateral ja e `Dashboard` para usuarios com `iluminacao.dashboard.ler`; apos o login, esses usuarios entram diretamente no Dashboard, enquanto usuarios sem essa permissao, como manutencao, entram no fluxo operacional de Iluminacao Publica. O modulo `Iluminacao Publica` permanece como area operacional propria, preservando resumo pequeno contextual, relatorio, lista, detalhe, historico, observacoes, alteracao normal de status, acoes administrativas restritas e rota/mapa operacional. O Dashboard geral nao cria, nesta fase, endpoints transversais como `/api/internal/dashboard/resumo`, `/api/internal/dashboard/widgets` ou `/api/internal/dashboard/series`; os endpoints atuais `/api/internal/iluminacao/dashboard/*` continuam corretos como fonte de widgets do modulo Iluminacao. Endpoints gerais agregadores ficam reservados para quando houver pelo menos um segundo modulo real com dados proprios.
...
Atualizacao documental 2026-06-23: a tela inicial do Geoportal Interno foi implementada no frontend interno em `/interno/` como Dashboard geral. Para usuarios com `iluminacao.dashboard.ler`, o menu passa a exibir o item `Dashboard`, a entrada apos login e o fluxo de widgets gerenciais; usuarios sem essa permissao continuam no fluxo operacional de Iluminacao Publica. Na v1, a tela consome os endpoints ja existentes `GET /api/internal/iluminacao/dashboard/resumo`, `GET /api/internal/iluminacao/dashboard/ranking` e `GET /api/internal/iluminacao/dashboard/series?granularidade=semana`; nao foi criado um endpoint geral `/api/internal/dashboard/*`.
...
Registro historico do recorte inicial: dashboard, mapa operacional, endpoints de estatisticas, endpoints de mapa, proxy, producao interna e botao publico de login eram etapas futuras. Posteriormente, proxy e producao interna foram ativados de forma controlada; a shell tambem passou a exibir coordenadas, rota Google Maps e mapa simples no detalhe. Na etapa anterior do commit `ebc5798`, os endpoints agregados read-only de Dashboard ainda existiam somente no backend e nao havia ainda o frontend do dashboard; o frontend foi implementado e publicado depois no commit `e5a488e`, com a shell interna em `/interno/` e a entrada `Dashboard` para usuarios com `iluminacao.dashboard.ler`. O mapa operacional amplo continua fora do MVP visual; os endpoints agregados read-only de Dashboard ja foram implementados localmente no backend e ainda dependem de permissao operacional antes de deploy.
...
Anexos, upload de fotos, mapa operacional amplo, evolucoes estatisticas avancadas e endpoints gerais agregadores, quando houver um segundo modulo real com dados proprios, ficam para etapas posteriores, com contrato, permissoes, GRANTs e validacoes proprias. A base backend read-only do Dashboard ja existe localmente e deve ser validada em servidor somente apos criacao/concessao controlada de `iluminacao.dashboard.ler`.
