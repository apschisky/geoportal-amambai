...
Dashboard gerencial read-only de Iluminacao Publica: a implementacao local dos endpoints `GET /api/internal/iluminacao/dashboard/resumo`, `GET /api/internal/iluminacao/dashboard/ranking` e `GET /api/internal/iluminacao/dashboard/series` usa a permissao especifica `iluminacao.dashboard.ler`. No modelo real de `mod_auth.permissoes`, essa permissao deve ser cadastrada como `modulo = 'iluminacao'` e `chave = 'dashboard.ler'`. Ela deve ser concedida apenas a perfil gerencial/administrativo autorizado e nao deve ser concedida ao perfil `manutencao-iluminacao`. Nesta etapa local nao houve migration, SQL, GRANT ou alteracao de banco; antes de deploy, a criacao/concessao da permissao deve ocorrer por procedimento operacional controlado.
