...
A primeira versao deve usar matriz conservadora: `aberta` pode ir para `em_triagem`, `cancelada` ou `indeferida`; `em_triagem` pode ir para `encaminhada`, `aguardando_material`, `nao_localizado`, `cancelada` ou `indeferida`; `encaminhada` pode ir para `em_execucao`, `aguardando_material`, `nao_localizado` ou `cancelada`; `em_execucao` pode ir para `aguardando_material`, `resolvida` ou `nao_localizado`; `aguardando_material` pode ir para `encaminhada`, `em_execucao` ou `cancelada`. `resolvida`, `cancelada`, `indeferida` e `nao_localizado` sao terminais nesta primeira versao e nao devem sair para outro status; reabertura ou correcao administrativa fica para fluxo futuro separado.
...
Decisao futura consolidada: correcao, volta de fase e reabertura de status nao entram no PATCH status normal. O contrato planejado deve ser `PATCH /api/internal/iluminacao/solicitacoes/{id}/status-correcao`, ou nome equivalente aprovado antes da implementacao, sempre separado do fluxo de manutencao.

Permissao planejada: `iluminacao.solicitacoes.corrigir_status`. No banco real, a representacao esperada e `modulo = 'iluminacao'` e `chave = 'solicitacoes.corrigir_status'`, pois `mod_auth.permissoes` usa colunas `modulo` e `chave`, e a aplicacao compoe a permissao efetiva como `lower(btrim(modulo)) || '.' || lower(btrim(chave))`. Essa permissao deve ser diferente de `iluminacao.solicitacoes.atualizar_status`, restrita a perfil administrativo/autorizado e nao deve ser atribuida ao perfil `manutencao-iluminacao`.

Payload planejado v1: aceitar somente `novo_status` e `justificativa`. Nao aceitar `tipo_correcao` como autoridade do frontend na primeira versao; o backend deve inferir se a operacao e `reabertura`, `volta_fase` ou `correcao_status` pelo status atual e novo status. Campos extras, dados pessoais, `finalizado_em`, prioridade, auditoria, SQL, role, GRANT, token, senha, cookie, hash, `session_secret` ou `DATABASE_URL` devem retornar 422.

Regras de autorizacao e seguranca: exigir sessao autenticada, `require_permission("iluminacao.solicitacoes.corrigir_status")`, header mutavel `X-Geoportal-Internal-Request: 1`, justificativa obrigatoria de 10 a 1000 caracteres apos trim e mensagens de erro sanitizadas. O frontend pode ocultar a acao, mas a regra real deve permanecer no backend.

Regras de negocio planejadas: permitir correcao entre status ativos, como `em_execucao -> encaminhada` ou `aguardando_material -> em_execucao`; permitir reabertura de terminal somente para status ativos controlados, preferencialmente `em_triagem`, `em_execucao` ou `aguardando_material`; evitar reabertura direta para `aberta`, salvo decisao administrativa explicita; permitir correcao entre terminais apenas para erro administrativo claro. O endpoint nao deve alterar prioridade, protocolo, geometria, dados pessoais, observacoes internas, `deleted_at` ou `deleted_reason`.

Regra planejada para `finalizado_em`: ao reabrir de terminal para ativo, limpar `finalizado_em` para `NULL`, preservando o fechamento anterior no historico; ao finalizar novamente, preencher novo timestamp. Em correcao entre ativos, manter `finalizado_em=NULL`; em correcao ativo -> terminal, preencher `finalizado_em=now()`; em correcao terminal -> terminal, manter o valor existente salvo decisao especifica futura.

Auditoria planejada: registrar historico na mesma transacao do UPDATE, com status anterior, novo status, usuario, justificativa em `observacao_resumida` e origem administrativa. Recomendacao: usar `origem_acao='ajuste_administrativo'`. Para `acao`, preferir `reabertura` quando sair de terminal para ativo, se aceito pelo schema; para demais correcoes, preferir `correcao_status` se aceito. Antes da implementacao, confirmar se esses valores ja existem nas constraints; se nao existirem, planejar migration separada.
...
Inventario de permissao: a permissao planejada `iluminacao.solicitacoes.corrigir_status` nao aparece nos scripts administrativos locais de bootstrap inspecionados e a consulta real de homologacao por `modulo = 'iluminacao'` e `chave = 'solicitacoes.corrigir_status'` retornou zero linhas. Nenhum perfil possui essa permissao hoje em homologacao. Ela deve ser criada e vinculada em ciclo separado por seed/script administrativo idempotente ou migration operacional controlada, seguindo o padrao do projeto. O perfil `manutencao-iluminacao` deve permanecer sem essa permissao; o perfil administrativo/autorizado deve recebe-la somente apos revisao humana.

Esse fluxo nao foi implementado nesta etapa. Producao, producao interna, Apache/proxy, frontend/tela interna, migrations, schema, `.env` versionado e NSSM nao foram alterados nesta etapa. Nenhum endpoint de reabertura/correcao, endpoint de anexos, usuario novo, perfil novo, permissao real nova, role nova ou GRANT foi criado; a API publica permaneceu preservada.
