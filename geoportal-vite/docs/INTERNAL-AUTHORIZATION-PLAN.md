...
Atualizacao documental 2026-06-23: a visibilidade do item `Dashboard` e dos widgets gerenciais do frontend interno depende da permissao `iluminacao.dashboard.ler`. Usuarios sem essa permissao, como `manutencao.producao`/`manutencao-iluminacao`, nao visualizam o Dashboard geral e continuam no fluxo operacional de Iluminacao Publica.
...
Dashboard gerencial read-only de Iluminacao Publica: a implementacao dos endpoints `GET /api/internal/iluminacao/dashboard/resumo`, `GET /api/internal/iluminacao/dashboard/ranking` e `GET /api/internal/iluminacao/dashboard/series` usa a permissao especifica `iluminacao.dashboard.ler`. Em produção interna, a permissao foi criada com `id=18`, `modulo='iluminacao'`, `chave='dashboard.ler'`, `descricao='Ler dashboard gerencial interno de solicitacoes de Iluminacao Publica.'` e `ativo=true`. A vinculacao foi feita apenas ao perfil `administrador-interno-geoportal`; a consulta para o perfil `manutencao-iluminacao` retornou `0 linhas`. O backup manual anterior a essa alteracao foi `C:\apps\geoportal-api\backups\manual\pre_dashboard_ler_mod_auth_20260622_101948.sql`. Nao houve migration estrutural, SQL adicional para o app nem alteracao de frontend para habilitar o acesso; a autorizacao real continua sendo controlada pelo backend.
...
Decisao consolidada: correcao, volta de fase e reabertura de status nao entram no PATCH status normal. O backend foi implementado no commit `313afd8 Implementa correcao administrativa de status` com o contrato `PATCH /api/internal/iluminacao/solicitacoes/{id}/status-correcao`, sempre separado do fluxo de manutencao, e validado em servidor/ambiente real da API interna de producao apos o marco documental `da4be5c`. Ainda falta frontend administrativo.

Permissao: `iluminacao.solicitacoes.corrigir_status`. No banco real, a representacao e `modulo = 'iluminacao'` e `chave = 'solicitacoes.corrigir_status'`, pois `mod_auth.permissoes` usa colunas `modulo` e `chave`, e a aplicacao compoe a permissao efetiva como `lower(btrim(modulo)) || '.' || lower(btrim(chave))`. Essa permissao foi adicionada ao bootstrap administrativo local no commit `313afd8`, e em producao interna foi criada/vinculada por procedimento operacional controlado com backup previo. Ela e diferente de `iluminacao.solicitacoes.atualizar_status`, fica restrita a perfil administrativo/autorizado e nao deve ser atribuida ao perfil `manutencao-iluminacao`.
...
Inventario de permissao: antes da implementacao local, a consulta real de homologacao por `modulo = 'iluminacao'` e `chave = 'solicitacoes.corrigir_status'` retornou zero linhas. O commit `313afd8` adicionou a permissao ao bootstrap administrativo local. Na validacao real de producao interna, a tentativa inicial de bootstrap com a role runtime da API falhou com permissao negada em `mod_auth.permissoes`, resultado coerente com menor privilegio: a role runtime nao deve ter escrita ampla permanente em tabelas de autorizacao. Em seguida, com backup previo `C:\apps\geoportal-api\backups\manual\pre_status_correcao_mod_auth_20260617_143551.sql`, a permissao foi criada/vinculada por procedimento operacional controlado usando usuario administrativo PostgreSQL. O perfil `administrador-interno-geoportal` recebeu `iluminacao.solicitacoes.corrigir_status`; o perfil `manutencao-iluminacao` permaneceu sem essa permissao, com conferencia retornando zero linhas.
...
- `GET /api/internal/admin/users/{usuario_id}/profiles`: consulta read-only protegida por `admin.usuarios.ler`, sem header mutavel, retornando somente `perfil_id`, `chave`, `nome`, `modulo`, `ativo` e `criado_em`;
- `POST /api/internal/admin/users/{usuario_id}/profiles/{perfil_id}/deactivate`: mutacao protegida por `admin.usuarios.remover_perfis`, com `X-Geoportal-Internal-Request: 1`, payload restrito a `modulo` e `justificativa`, justificativa obrigatoria de 10 a 1000 caracteres e campos extras proibidos.

A permissao nova `admin.usuarios.remover_perfis` foi adicionada ao bootstrap administrativo idempotente. Ela deve ser vinculada somente ao perfil administrativo autorizado. Perfis operacionais, incluindo `manutencao-iluminacao`, nao recebem `admin.*`.

A desativacao e logica (`ativo=false`) em `mod_auth.usuario_perfis`; nao ha `DELETE`, migration estrutural ou alteracao de schema. Vinculo inexistente retorna `404`; vinculo ja inativo retorna `409`. Negativas por auto-rebaixamento e protecao do ultimo administrador retornam `403 {"detail":"Forbidden"}` com auditoria interna.
...
## Homologacao - bootstraps RBAC de consulta global e administracao de Iluminacao

Homologacao operacional registrada no servidor alinhado ao commit `bd50401 Garante auth me nos perfis de autorizacao`. Antes dele, o commit `52aa123 Garante permissao do dashboard no bootstrap admin` corrigiu o bootstrap administrativo para garantir a permissao `iluminacao.dashboard.ler`, usada pelos endpoints do dashboard gerencial.

A homologacao ocorreu no banco `amambaiGis_homologacao`, com backup manual previo `C:\apps\geoportal-api\backups\manual\pre_bootstrap_perfis_autorizacao_amambaiGis_homologacao_20260629_080619.sql`, tamanho `249012192` bytes. No servidor, os testes focados `tests/test_bootstrap_internal_admin_profile_admin.py`, `tests/test_bootstrap_internal_authorization_profiles_admin.py` e `tests/test_bootstrap_internal_maintenance_profile_admin.py` executaram 39 itens sem falha reportada.

Fase 1: foram aplicados GRANTs temporarios em homologacao para `mod_auth.permissoes` e `mod_auth.perfil_permissoes`; em seguida foi executado `bootstrap_internal_admin_profile.py --login admin.homologacao`. A permissao `iluminacao.dashboard.ler` foi garantida, vinculada ao perfil `administrador-interno-geoportal` e nao vinculada ao perfil `manutencao-iluminacao`. Os GRANTs temporarios foram revogados ao final e os privilegios finais ficaram fechados.

Fase 2: foram aplicados GRANTs temporarios minimos para criar perfis e vinculos; em seguida foi executado `bootstrap_internal_authorization_profiles.py --profile all`. Foram criados em homologacao os perfis `gestor-consulta-global` (`id=5`, ativo) e `administrador-modulo-iluminacao` (`id=6`, ativo). Ambos ficaram sem permissoes `admin.*`. Os GRANTs temporarios foram revogados e os privilegios finais ficaram fechados.

Correcao posterior: a validacao identificou que `administrador-modulo-iluminacao` precisava garantir o vinculo `internal.auth.me`. O commit `bd50401` corrigiu bootstrap e teste; o rerun controlado em homologacao completou o vinculo faltante. A validacao final confirmou `administrador-modulo-iluminacao` com `admin_permissoes=0`, `auth_me=1` e `prioridade=1`, e `gestor-consulta-global` com `admin_permissoes=0` e `auth_me=1`.

O dry-run final de `administrador-modulo-iluminacao` lista `internal.auth.me`, `iluminacao.dashboard.ler`, `iluminacao.solicitacoes.ler`, `iluminacao.solicitacoes.ver_historico`, `iluminacao.solicitacoes.ver_observacoes`, `iluminacao.solicitacoes.comentar`, `iluminacao.solicitacoes.atualizar_status`, `iluminacao.solicitacoes.atualizar_prioridade` e `iluminacao.solicitacoes.corrigir_status`.
