...
Validacao sanitizada do usuario operacional de producao: `manutencao.producao` autenticou com sucesso, `/api/internal/auth/me` retornou os campos `authenticated`, `usuario_id` e `permissoes`, e as permissoes efetivas foram `internal.auth.me`, `iluminacao.solicitacoes.ler`, `iluminacao.solicitacoes.ver_observacoes`, `iluminacao.solicitacoes.comentar` e `iluminacao.solicitacoes.atualizar_status`. A resposta nao concedeu `admin.*` nem `iluminacao.solicitacoes.atualizar_prioridade`. O logout tambem foi validado. Como o contrato atual de `/me` nao retorna `login`, `nome` e `perfis`, a shell pode exibir fallback como `Usuario interno #2`; isso nao e falha de autenticacao.
