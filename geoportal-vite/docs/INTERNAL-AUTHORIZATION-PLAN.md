...
Permissao planejada: `iluminacao.solicitacoes.corrigir_status`. Essa permissao deve ser diferente de `iluminacao.solicitacoes.atualizar_status`, restrita a perfil administrativo/autorizado e nao deve ser atribuida ao perfil `manutencao-iluminacao`.
...
Inventario de permissao: a permissao planejada `iluminacao.solicitacoes.corrigir_status` nao aparece nos scripts administrativos locais de bootstrap inspecionados. Ela deve ser criada e vinculada em ciclo separado por seed/script administrativo idempotente ou migration operacional controlada, seguindo o padrao do projeto. O perfil `manutencao-iluminacao` deve permanecer sem essa permissao; o perfil administrativo/autorizado deve recebe-la somente apos revisao humana.
