...
Plano de autorizacao para o frontend administrativo de `status-correcao`: a acao visual deve ser renderizada somente quando `/api/internal/auth/me` retornar `iluminacao.solicitacoes.corrigir_status` em `permissoes`. A acao nao deve aparecer para `manutencao-iluminacao`, `manutencao.producao` ou qualquer usuario sem essa permissao especifica. O bloco visual deve ficar separado do PATCH normal de status, com rotulo administrativo claro, confirmacao forte, justificativa obrigatoria e chamada mutavel com `X-Geoportal-Internal-Request: 1`. Mesmo que o frontend oculte a acao, a autorizacao real continua no backend por `require_permission("iluminacao.solicitacoes.corrigir_status")`, payload restrito e mensagens de erro sanitizadas. A primeira UI nao deve incluir administracao de usuarios/perfis/permissoes, dashboard avancado, edicao direta de banco, acao em lote ou atalho para manutencao.
