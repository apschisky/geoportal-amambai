...
Perfil operacional de manutencao de Iluminacao: o script administrativo `geoportal-backend/scripts/admin/bootstrap_internal_maintenance_profile.py` foi preparado para homologacao interna, de forma idempotente e sem criar usuario. Ele garante o perfil `manutencao-iluminacao` (`Manutencao - Iluminacao Publica`) para um usuario existente informado por `--login`, com as permissoes minimas `internal.auth.me`, `iluminacao.solicitacoes.ler`, `iluminacao.solicitacoes.ver_observacoes`, `iluminacao.solicitacoes.comentar` e `iluminacao.solicitacoes.atualizar_status`. O perfil nao inclui `admin.*` nem `iluminacao.solicitacoes.atualizar_prioridade`, pois a finalidade inicial e validar o modo visual operacional de manutencao e aplicar menor privilegio. O script aceita `--dry-run`, nao pede senha, nao imprime segredos, nao remove permissoes/perfis existentes e reutiliza o bootstrap transacional de perfis/permissoes.
...
Decisao futura: correcao ou reversao de status nao entra no PATCH status normal. Deve ser fluxo separado, muito controlado, com justificativa obrigatoria, permissao especifica diferente de `iluminacao.solicitacoes.atualizar_status`, restrita a poucos perfis autorizados, e auditoria propria no historico. Conforme contrato futuro, pode usar `origem_acao='ajuste_administrativo'` e/ou `acao='reabertura'`, respeitando os valores permitidos pela migration. Esse fluxo nao foi implementado nesta etapa e a tela futura nao deve replicar essa regra livremente; a regra deve permanecer no backend.
