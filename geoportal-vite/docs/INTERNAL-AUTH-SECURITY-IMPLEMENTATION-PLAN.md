...
Ainda não há endpoint interno de login exposto, endpoint interno protegido ativo no app principal, usuário real, sessão real criada por endpoint, token real, cookie real, CSRF, JWT ou middleware de autenticação. O router técnico de smoke e a feature flag existem, mas o router não está incluído no app principal.
Próximos passos: conectar a feature flag ao `include_router` em tarefa controlada, validando primeiro com flag desligada; esta etapa crítica deve seguir Codex High.
...
13. → Implementar atraso progressivo e bloqueio temporário persistente integrados.
14. → Conectar feature flag ao registro controlado de rotas internas.
15. → Criar testes automatizados de acesso negado/autorizado com rota controlada, dependency ou middleware.
