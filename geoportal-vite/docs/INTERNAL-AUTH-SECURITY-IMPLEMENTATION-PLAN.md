...
A base estrutural inicial do schema `mod_auth` ja foi criada, aplicada e documentada em homologacao e producao pelas migrations `0006` a `0009`. O endpoint interno de login existe apenas sob feature flag e ainda nao ha tela interna, cookie real, CSRF, JWT, role runtime real, token real operacional ou seed.
...
Validação local desta etapa: `tests/test_auth_login_audit_repository.py` passou com 6 testes; `tests/test_reset_internal_user_password_admin.py` passou com 12 testes; `tests/test_internal_auth_login_router.py` passou com 5 testes; `tests/test_internal_routes_feature_flag.py`, `tests/test_internal_routes_config.py` e `tests/test_internal_auth_smoke_router.py` passaram; `tests/test_auth_service.py` passou com 26 testes incluindo validação de robustez da atualização best effort de `ultimo_login_em`; `tests/test_auth_user_repository.py` passou com 9 testes; suite completa local passou com 283 testes.
...
O endpoint interno `POST /api/internal/auth/login` existe somente sob `GEOPORTAL_INTERNAL_ROUTES_ENABLED`; não há endpoint de negócio interno, cookie real, CSRF, JWT ou middleware de autenticação global. O usuário `admin.homologacao` existe somente em homologação por bootstrap administrativo controlado. O router técnico de smoke e o endpoint de login só ficam ativos com feature flag ligada explicitamente.
Próximos passos: manter a flag desligada em produção; homologação permanece como ambiente controlado para smoke test protegido e login interno inicial; ainda sem cookie real, CSRF, JWT, endpoint `/me` ou endpoint de negócio interno.
...
Decisão arquitetural sobre transporte final e CSRF (etapa documental):

A validação técnica com `Authorization: Bearer` e token retornado no corpo foi aceita apenas como validação intermediária isolada em homologação. Para uso real em navegador por usuários finais do Geoportal Interno, a decisão recomendada é implementar **cookie HttpOnly + Secure + SameSite** em etapa separada com CSRF (token CSRF separado, double-submit, header customizado, validação de Origin/Referer ou combinação) e logout (revogação em `mod_auth.sessoes` sem DELETE físico). Detalhes em `geoportal-vite/docs/INTERNAL-AUTH-TECHNICAL-DECISIONS.md` seção "Decisão arquitetural sobre transporte final e CSRF". Implementação de cookie/CSRF/logout será etapa separada com Codex High, testes e validação operacional em homologação. Não liberar tela interna para usuários reais antes dessa etapa final. Bearer pode continuar para testes técnicos ou clientes não navegador se permitido pela arquitetura, mas não como fluxo principal.
...
- Testes automatizados de autenticação passaram localmente com 281 testes no total, incluindo 12 testes do reset administrativo, 5 testes do endpoint interno de login, 26 `test_auth_service` com robustez de `ultimo_login_em` best effort, 17 `test_create_internal_user` e 9 `test_auth_user_repository`.
...
- Naquela etapa, nenhum usuario real, endpoint de login, sessao real, token real, cookie real, CSRF ou JWT havia sido criado. Em etapa posterior de bootstrap controlado, o primeiro usuario `admin.homologacao` foi criado somente em homologacao.
...
### Pendente
- Validar endpoint de login em homologacao controlada.
