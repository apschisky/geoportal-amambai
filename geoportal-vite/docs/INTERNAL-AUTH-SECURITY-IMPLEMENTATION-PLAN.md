...
Pré-requisitos que ainda precisam ser fechados antes do primeiro CRUD administrativo:
- Rate limit de login por IP e por IP+login.
- Tratamento seguro do IP real atrás do Apache/proxy, com `X-Forwarded-For` e regra operacional clara.
- Testes automatizados para acesso negado, bloqueio, expiração e revogação de sessão.
- Anti-elevação, proteção contra remover o último administrador e auditoria de ações administrativas.
- Separação forte entre permissões de negócio e permissões administrativas, sem hardcoded bypass por login específico.
- Proteção de infraestrutura contra abuso volumétrico; a camada FastAPI não substitui WAF, proxy, rate limit de infraestrutura e controles de rede.

Conclusão operacional: a Etapa 0 é pré-requisito documental e operacional antes do CRUD administrativo; até ela ser concluída e validada em homologação, a exposição administrativa deve permanecer fechada.
