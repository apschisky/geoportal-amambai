...
## Etapa 0 — Segurança administrativa antes do CRUD

Antes de qualquer endpoint mutável de usuários, perfis, permissões ou tela administrativa, o projeto deve concluir uma etapa preventiva de segurança chamada Etapa 0.

Ponto de auditoria atual:
- Há controles iniciais relevantes já documentados e validados: rotas internas sob feature flag, cookie HttpOnly/Secure/SameSite para navegador, revogação lógica de sessão no logout, respostas 401/403 genéricas, ausência de segredo/token no frontend e autorização por permissão em endpoints internos.
- O projeto ainda não deve tratar isso como um “admin console” aberto nem como um CRUD administrativo liberado.

Reforço recente da Etapa 0 (commits `152c177` e `f3d8ff3`): o primeiro bloqueio de bruteforce/credential stuffing básico do login interno foi implementado localmente e enviado ao GitHub. O reforço registra:
- helper de resolução segura de IP em `geoportal-backend/app/core/client_ip.py`;
- confiança conservadora em `X-Forwarded-For`/`X-Real-IP` apenas quando o peer imediato é confiável e o valor é único e válido;
- rate limit do login interno por IP, por login/origem e por IP+login, com limites de 5/15 min, 20/15 min e 5/15 min respectivamente;
- uso persistente da auditoria existente em `mod_auth.login_auditoria` sem nova migration;
- resposta `429` sanitizada para excesso de tentativas, sem revelar usuário, contador ou escopo bloqueado;
- suporte a `RATE_LIMIT_ENABLED=false` para desligar o rate limit sem perder a autenticação comum.

Pré-requisitos ainda pendentes antes do primeiro CRUD administrativo:
- validar em servidor/homologação/produção interna se o Apache realmente encaminha os headers esperados para o backend;
- validar no runtime se o backend enxerga o IP real ou apenas `127.0.0.1`;
- validar se `RATE_LIMIT_ENABLED=true` está ativo no ambiente interno e se o `429` aparece corretamente sem expor usuário;
- testes automatizados adicionais para acesso negado, bloqueio, expiração e revogação de sessão;
- anti-elevação, proteção contra remover o último administrador efetivo e auditoria administrativa própria;
- separação forte entre permissões de negócio e administrativas, sem hardcoded bypass por login específico;
- proteção de infraestrutura contra abuso volumétrico; a camada FastAPI não resolve DDoS volumétrico sozinho.
...
1. Fazer pull controlado no servidor e executar os testes focados antes de qualquer restart.
2. Validar em homologação os headers efetivos do Apache, o IP resolvido, `RATE_LIMIT_ENABLED=true`, o contrato `401/429` e a auditoria, sem registrar IP bruto ou segredo.
3. Implementar auditoria administrativa própria para alterações de usuários, perfis e permissões.
4. Definir e testar regras contra autoelevação, alteração indevida do próprio perfil crítico e bloqueio/remoção do último administrador efetivo.
5. Avaliar validação de `Origin`/`Referer` ou CSRF token real para ações administrativas críticas, mantendo o header mutável existente.
6. Consolidar permissões administrativas granulares e implementar primeiro endpoints read-only.
7. Somente depois liberar endpoints administrativos mutáveis e planejar o frontend administrativo.

Este reforço não foi validado nem publicado no runtime interno por este ciclo documental. O estado correto é: código implementado nos commits `152c177` e `f3d8ff3`, enviado ao repositório remoto e pendente de pull/validação operacional controlada no servidor.
