...
Validação no servidor: git pull aplicado; testes no servidor passaram; homologação, produção local e produção pública foram reiniciadas e validadas. A API pública continuou saudável em todos os ambientes. Com a flag desligada/ausente, `/api/internal/auth/smoke` retornou `404` em homologação e em produção pública.
...
Próximos passos: manter a flag desligada em homologação e produção, e ativar somente em homologação para smoke controlado; ainda sem login, usuário real, cookie real, CSRF ou endpoint `/me`.
...
### Pendente
- Ativar a flag somente em homologação para smoke controlado.
- Configurar `GEOPORTAL_INTERNAL_SESSION_SECRET` somente no servidor, fora do Git.
- Testar `/api/internal/auth/smoke` em homologação com a feature flag ligada.
