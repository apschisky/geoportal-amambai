...
Sequencia segura recomendada a partir daqui: planejar endpoint de atribuicao de perfil ao usuario, protegido por permissao especifica como `admin.usuarios.atribuir_perfis` e header mutavel; validar que usuario sem perfil passa a receber permissoes apos atribuicao; manter bloqueio/desbloqueio e reset de senha para etapas posteriores; depois criar o primeiro endpoint interno de negocio do modulo Iluminacao; tela interna continua etapa posterior. Producao permanece sem alteracao nesta etapa.
