...
- planejar hardening explícito de IP real no Apache/proxy, caso seja necessário separar clientes pelo IP original, e validar novamente o runtime após essa decisão;
...
3. Definir e testar regras contra autoelevação, alteração indevida do próprio perfil crítico e bloqueio/remoção do último administrador efetivo.
4. Avaliar validação de `Origin`/`Referer` ou CSRF token real para ações administrativas críticas, mantendo o header mutável existente.
5. Consolidar permissões administrativas granulares e implementar primeiro endpoints read-only.
6. Somente depois liberar endpoints administrativos mutáveis e planejar o frontend administrativo.

Validação operacional em produção interna: o servidor foi atualizado até `bf7b4df`, a suíte completa passou com `695 passed` e `3 warnings`, e `GeoportalAPIInternaProducao` foi validado e reiniciado de forma controlada pelo harness em `127.0.0.1:8003`. O login normal de `admin.producao` e o logout foram confirmados. Um login fictício de probe produziu `401,401,401,401,401,429`, validando o bloqueio sanitizado. A configuração ativa pesquisada do Apache não apresentou `X-Forwarded-For` ou `X-Real-IP` explícitos; portanto, o reforço está publicado e funcional, mas o IP real por cliente permanece hardening futuro, sem enfraquecer o fallback conservador do backend.
