...
O serviço interno de hash/verificação de senha usando Argon2id foi implementado e validado localmente e no servidor. O serviço interno de sessão opaca/token também foi implementado e validado localmente e no servidor. O repository interno de usuários foi criado para `mod_auth.usuarios` e o repository interno de sessões foi criado para operar com `mod_auth.sessoes`, `token_hash`, expiração e revogação por `revogado_em`, sem `DELETE`. Ainda não há login funcional, endpoint interno de autenticação ou middleware de autorização exposto.
