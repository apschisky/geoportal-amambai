...
A validação técnica com `Authorization: Bearer` e token retornado no corpo foi aceita apenas para validação intermediária isolada em homologação. Para uso real em navegador por usuários finais, a decisão recomendada é implementar **cookie HttpOnly + Secure + SameSite** com proteção CSRF (token CSRF, double-submit, header customizado, validação de Origin/Referer ou combinação) e logout em etapa separada. Detalhes técnicos em `geoportal-vite/docs/INTERNAL-AUTH-TECHNICAL-DECISIONS.md` seção "Decisão arquitetural sobre transporte final e CSRF". Implementação será etapa futura com Codex High, testes e validação operacional. Ainda não há cookie real, CSRF implementado ou logout para usuários. Bearer pode continuar para testes técnicos ou APIs não navegador, mas não como fluxo principal.
