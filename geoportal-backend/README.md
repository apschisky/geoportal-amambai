...
Endpoint administrativo de detalhe somente leitura: `GET /api/internal/admin/users/{usuario_id}` foi criado sob a mesma feature flag interna e protegido por `require_permission("admin.usuarios.ler")`. Ele consulta apenas `mod_auth.usuarios` por `id` com bind parameter e retorna `usuario` com campos seguros: `id`, `login`, `nome`, `email`, `ativo`, `bloqueado` booleano e `criado_em`. Sem sessao retorna 401 generico, sem permissao retorna 403 generico e usuario inexistente retorna 404 generico. A resposta nao retorna senha, `senha_hash`, token, `token_hash`, cookie, `session_secret`, `DATABASE_URL`, dados de sessao, auditoria, SQL, role, GRANT, `bloqueado_ate`, `atualizado_em` ou `ultimo_login_em`. Por ser GET de consulta, nao exige `X-Geoportal-Internal-Request`. Esta etapa nao criou endpoint mutavel, nao cria/edita/bloqueia/desbloqueia usuario, nao redefine senha, nao atribui perfil, nao alterou schema, nao criou migration, nao alterou producao e nao criou tela.

Validacao operacional de GET /api/internal/admin/users em homologacao: o commit `119390e` Adiciona listagem interna de usuarios foi aplicado no servidor e validado com pytest completo: 347 passed. A validacao ocorreu em processo isolado de homologacao com variaveis temporarias; todas foram limpas ao final. Sem sessao, o endpoint retornou 401 com `{'detail': 'Not authenticated'}`. Com admin.homologacao autenticado (que possui a permissao admin.usuarios.ler), o endpoint retornou 200 com lista de usuarios. Resultado sanitizado: `login_status=200`, `login_set_cookie=True`, `cookie_jar_tem_sessao=True`, `users_status=200`, `users_tem_lista=True`, `users_total=1`, `users_tem_admin_homologacao=True`. Campos retornados em cada usuario: `id`, `login`, `nome`, `email`, `ativo`, `bloqueado`, `criado_em`. A resposta nao expôs senha, `senha_hash`, token, `token_hash`, cookie, `session_secret`, `DATABASE_URL`, SQL, role, GRANT, dados de sessao, auditoria ou `bloqueado_ate`. Variaveis temporarias foram limpas apos o teste. Producao, NSSM, `.env` versionado e frontend nao foram alterados. Nenhum endpoint mutavel foi criado nesta etapa.

Validacao operacional de GET /api/internal/admin/users/{usuario_id} em homologacao: o commit `ea4e457` Adiciona detalhe interno de usuario foi aplicado no servidor e validado com pytest completo: 358 passed. A validacao ocorreu em processo isolado de homologacao com variaveis temporarias; todas foram limpas ao final. Sem sessao, o endpoint retornou 401 com `{'detail': 'Not authenticated'}`. Com admin.homologacao autenticado (que possui a permissao admin.usuarios.ler), o endpoint retornou 200 para o usuario_id=7 (admin.homologacao). Resultado sanitizado: `login_status=200`, `login_set_cookie=True`, `cookie_jar_tem_sessao=True`, `detail_status=200`, `detail_tem_usuario=True`, `detail_usuario_id=7`, `detail_usuario_login=admin.homologacao`, `detail_campos=['ativo', 'bloqueado', 'criado_em', 'email', 'id', 'login', 'nome']`. Usuario inexistente retornou 404 generico `{'detail': 'Not found'}`. A resposta nao expôs senha, `senha_hash`, token, `token_hash`, cookie, `session_secret`, `DATABASE_URL`, SQL, role, GRANT, dados de sessao, auditoria, `bloqueado_ate`, `atualizado_em` ou `ultimo_login_em`. Variaveis temporarias foram limpas apos o teste. Producao, NSSM, `.env` versionado e frontend nao foram alterados. Nenhum endpoint mutavel foi criado nesta etapa.

Plano do primeiro endpoint administrativo mutavel: o proximo endpoint planejado e `POST /api/internal/admin/users`, a ser implementado em etapa separada com Codex High. Ele deve ficar sob `GEOPORTAL_INTERNAL_ROUTES_ENABLED`, exigir sessao autenticada, `require_permission("admin.usuarios.criar")`, header `X-Geoportal-Internal-Request: 1` e a protecao CSRF/equivalente definida para rotas internas mutaveis. O payload planejado contem somente `login` obrigatorio, `nome` obrigatorio, `email` opcional e `senha_inicial` obrigatoria; devem ser proibidos `id`, `ativo`, `bloqueado`, `bloqueado_ate`, `senha_hash`, `perfil`, `perfis`, `permissoes`, `role`, `token`, `session_secret`, `DATABASE_URL`, campos de auditoria e campos de sessao. Validacoes previstas: `login` com `strip`, obrigatorio, nao vazio e unico case-insensitive; `nome` obrigatorio e nao vazio; `email` opcional, validado quando informado e unico conforme regra existente; `senha_inicial` obrigatoria e conforme politica de senha existente; rejeicao de campos extras se o padrao Pydantic permitir; sem login hardcoded e sem aceitar perfil/permissao no mesmo endpoint. Persistencia planejada: criar registro em `mod_auth.usuarios`, gerar `senha_hash` Argon2id com utilitario existente, `ativo=true`, `desativado_em=NULL`, `bloqueado_ate=NULL`, `atualizado_em=NULL` ou conforme padrao existente, sem criar sessao, perfil, vinculo `usuario_perfis`, permissao, auditoria de login, e sem apagar nada. Resposta 201 planejada: `usuario.id`, `usuario.login`, `usuario.nome`, `usuario.email`, `usuario.ativo`, `usuario.bloqueado`, `usuario.criado_em`; a resposta nao deve retornar senha, `senha_inicial`, `senha_hash`, token, `token_hash`, cookie, `session_secret`, `DATABASE_URL`, SQL, role, GRANT, sessao, auditoria, `bloqueado_ate`, `atualizado_em` ou `ultimo_login_em`. Erros planejados: 401 `Not authenticated`, 403 `Forbidden`, 409 `Conflict` para login/e-mail duplicado e 422 para payload invalido, sempre com mensagens genericas e sanitizadas, sem tabela, SQL ou constraint. Esta etapa documental nao cria endpoint, usuario real, perfil, permissao, role, GRANT, migration, schema, producao, NSSM, `.env`, frontend ou tela; apos a criacao, bloqueio/desbloqueio, reset de senha, atribuicao de perfil e primeiro endpoint interno de negocio, como Iluminacao, ficam para etapas separadas.

Primeiro endpoint administrativo mutavel implementado: `POST /api/internal/admin/users` cria somente usuario interno basico, sob `GEOPORTAL_INTERNAL_ROUTES_ENABLED`, com sessao autenticada, `require_permission("admin.usuarios.criar")` e header `X-Geoportal-Internal-Request: 1`. O endpoint aceita apenas `login`, `nome`, `email` opcional e `senha_inicial`; rejeita campos extras como `perfil`, `perfis`, `permissoes`, `ativo`, `bloqueado`, tokens, secrets, auditoria ou sessao. A senha inicial e usada apenas para gerar `senha_hash` Argon2id no backend; a resposta 201 retorna somente `usuario` com `id`, `login`, `nome`, `email`, `ativo`, `bloqueado` e `criado_em`. Conflitos de login/e-mail retornam 409 generico, payload invalido retorna 422, sem detalhes de tabela, SQL ou constraint. Esta implementacao nao atribui perfil, nao cria sessao, nao escreve em `login_auditoria`, nao envia e-mail, nao cria endpoint de bloqueio/reset/perfis, nao altera schema, nao cria migration, nao altera producao, NSSM, `.env`, frontend ou tela.

Politica de senha inicial administrativa: a validacao foi centralizada em `app/security/passwords.py` e aplicada antes de `hash_password(...)`. A senha inicial e obrigatoria, apos `strip` deve ter entre 6 e 128 caracteres, conter pelo menos uma letra e um numero, nao pode ser igual ao login ou ao nome normalizados e bloqueia uma lista curta de senhas comuns. Falhas retornam erro generico de payload invalido sem incluir a senha, `senha_inicial`, hash ou detalhes internos na resposta.

Validacao operacional de POST /api/internal/admin/users em homologacao: o commit `99f2987` Reforca politica de senha interna foi aplicado no servidor e validado com pytest completo: 403 passed. Antes da operacao real foram feitos backup de roles e backup custom do banco de homologacao. A role runtime `geoportal_api_homolog` recebeu apenas o minimo necessario para criacao via endpoint: `INSERT` em `mod_auth.usuarios` e `USAGE`/`SELECT` em `mod_auth.usuarios_id_seq`; a validacao final confirmou `usuarios_select=t`, `usuarios_insert=t`, `usuarios_update=t`, `usuarios_delete=f`, `usuarios_seq_usage=t` e `usuarios_seq_select=t`. O teste foi executado em processo isolado de homologacao com variaveis temporarias, limpas ao final. Cenários validados: 401 sem sessao; 403 sem header mutavel; 422 para senha invalida/fraca; 201 para criacao valida; 409 para duplicidade; 200 no detalhe do usuario criado; 200 no login do usuario criado. O usuario `teste.criacao` foi criado em homologacao com `id=8`, `ativo=true`, `bloqueado=false`, `email=NULL` e `criado_em` presente. Ele nao recebeu perfil automaticamente; `/api/internal/auth/me` para esse usuario retornou `permissoes=[]`. A resposta nao retornou senha real, `senha_inicial`, `senha_hash`, token, `token_hash`, cookie, `session_secret`, `DATABASE_URL`, SQL, role, GRANT, sessao, auditoria ou `bloqueado_ate`. Producao, NSSM, `.env` versionado e frontend nao foram alterados; nenhuma sessao foi criada automaticamente no ato da criacao administrativa. Proximas etapas: planejar endpoint de atribuicao de perfil com permissao especifica como `admin.usuarios.atribuir_perfis`, exigir header mutavel, validar que usuario sem perfil passa a receber permissoes apos atribuicao, e manter bloqueio/desbloqueio e reset de senha para etapas posteriores.

Contrato planejado para atribuicao de perfil: o endpoint futuro sera `POST /api/internal/admin/users/{usuario_id}/profiles`, sob `GEOPORTAL_INTERNAL_ROUTES_ENABLED`, exigindo sessao autenticada, `require_permission("admin.usuarios.atribuir_perfis")` e header `X-Geoportal-Internal-Request: 1`. Para o primeiro ciclo, a requisicao atribui apenas um perfil por vez, com payload permitido `perfil_id` obrigatorio e `modulo` opcional/nulo; sao proibidos `usuario_id` no corpo, `perfil_chave`, `permissoes`, `login`, senha, `senha_hash`, token, role, GRANT, `session_secret`, `DATABASE_URL`, auditoria e sessao. Comportamento planejado: 201 quando criar vinculo ativo, 200 quando o vinculo ativo ja existir sem duplicar, 401 sem sessao, 403 sem permissao, 403 sem header mutavel, 404 generico se usuario ou perfil nao existir e 422 para payload invalido. A primeira versao nao deve reativar vinculo inativo automaticamente, nao remover perfil, nao criar perfil, nao criar permissao, nao alterar senha, nao criar sessao, nao enviar e-mail, nao implementar batch e nao alterar producao. Resposta planejada em envelope: `vinculo.usuario_id`, `vinculo.perfil_id`, `vinculo.modulo`, `vinculo.ativo`. A tela futura podera exibir perfis em checkboxes e chamar o endpoint uma vez por perfil marcado; endpoint batch/transacional fica para fase futura com auditoria definida. A validacao em homologacao deve usar `teste.criacao`: antes da atribuicao, `/me` retorna `permissoes=[]`; apos atribuir perfil, `/me` deve retornar as permissoes do perfil.

Endpoint mutavel de atribuicao de perfil implementado: `POST /api/internal/admin/users/{usuario_id}/profiles` atribui um unico perfil por requisicao, protegido por `require_permission("admin.usuarios.atribuir_perfis")`, sessao autenticada e header `X-Geoportal-Internal-Request: 1`. O endpoint aceita apenas `perfil_id` positivo e `modulo` opcional/nulo, usa bind parameters, valida existencia de usuario e perfil ativo, cria vinculo em `mod_auth.usuario_perfis` quando ausente, retorna 200 se o vinculo ativo ja existir sem duplicar e retorna 409 generico se encontrar vinculo inativo, sem reativar automaticamente. A resposta e sanitizada em envelope `vinculo` com `usuario_id`, `perfil_id`, `modulo` e `ativo`. A implementacao nao cria perfil, permissao, usuario, sessao ou auditoria, nao remove perfil, nao implementa batch, nao usa `DELETE`, nao altera schema/migration, nao altera producao, NSSM, `.env`, frontend ou tela. A proxima etapa operacional e validar em homologacao com `teste.criacao` e confirmar que `/api/internal/auth/me` passa de `permissoes=[]` para as permissoes do perfil atribuido.

Validacao operacional de atribuicao de perfil em homologacao: o commit `092b5bb` Adiciona atribuicao interna de perfil foi aplicado no servidor e validado com pytest completo: 426 passed. Para a validacao, a role `geoportal_api_homolog` recebeu apenas o minimo adicional necessario: `INSERT` em `mod_auth.usuario_perfis`. A matriz final confirmou `usuario_perfis_select=t`, `usuario_perfis_insert=t`, `usuario_perfis_update=f`, `usuario_perfis_delete=f`, alem de `usuarios_select=t`, `perfis_select=t`, `permissoes_select=t` e `perfil_permissoes_select=t`. Com o usuario `teste.criacao` (`id=8`), antes da atribuicao `/api/internal/auth/me` retornava `permissoes=[]`. Foram validados: 401 sem sessao; 403 sem header mutavel; 201 na atribuicao valida com `usuario_id=8`, `perfil_id=3`, `modulo=None`, `ativo=True`; 200 na repeticao idempotente; 404 para perfil inexistente; 422 para payload invalido. Depois da atribuicao, `/api/internal/auth/me` para `teste.criacao` retornou 10 permissoes do perfil, incluindo `internal.auth.me` e `admin.usuarios.ler`. A verificacao `response_tem_senha=True` ocorreu apenas porque a permissao tecnica `admin.usuarios.redefinir_senha` contem a palavra `senha`; nao houve exposicao de senha real, `senha_hash`, token, `token_hash`, cookie, `session_secret`, `DATABASE_URL`, SQL, role, GRANT, sessao ou auditoria. Variaveis temporarias foram limpas. Producao, NSSM, `.env` versionado e frontend nao foram alterados. Nenhum batch, remocao de perfil, bloqueio/desbloqueio ou endpoint de reset de senha foi criado. Proximas etapas: planejar bloqueio/desbloqueio de usuario, reset de senha via endpoint, listagem de perfis para tela futura e, depois, primeiro modulo de negocio como Iluminacao.

Endpoint administrativo de perfis somente leitura: `GET /api/internal/admin/profiles` foi criado sob `GEOPORTAL_INTERNAL_ROUTES_ENABLED` e protegido por `require_permission("admin.perfis.ler")`. Ele lista apenas perfis ativos de `mod_auth.perfis`, usando SQL parametrizado, ordenacao estavel por nome/chave/id e resposta sanitizada com `id`, `chave`, `nome`, `ativo` e `criado_em`. Por ser GET de consulta, nao exige `X-Geoportal-Internal-Request`. A rota nao retorna permissoes detalhadas do perfil, usuarios, sessoes, auditoria, senha, `senha_hash`, token, `token_hash`, cookie, `session_secret`, `DATABASE_URL`, SQL, role ou GRANT. Esta etapa nao cria endpoint mutavel, nao cria/edita/remove perfil, nao cria permissao, usuario ou vinculo, nao altera schema, nao cria migration, nao altera producao, NSSM, `.env`, frontend ou tela.

Validacao operacional de GET /api/internal/admin/profiles em homologacao: o commit `93d96f4` Adiciona listagem interna de perfis foi aplicado no servidor e validado com pytest completo: 439 passed. A validacao ocorreu em processo isolado de homologacao com variaveis temporarias; todas foram limpas ao final. Sem sessao, o endpoint retornou 401 com `{'detail': 'Not authenticated'}`. Com admin.homologacao autenticado e com `admin.perfis.ler`, o endpoint retornou 200. Resultado sanitizado: `profiles_status=200`, `profiles_tem_lista=True`, `profiles_total=1`, `profiles_tem_admin_interno=True`. Campos retornados: `id`, `chave`, `nome`, `ativo`, `criado_em`. A resposta nao expôs senha, `senha_hash`, token, `token_hash`, cookie, `session_secret`, `DATABASE_URL`, SQL, role, GRANT, sessao, auditoria, permissoes, `perfil_permissoes` ou usuarios. Producao, NSSM, `.env` versionado e frontend nao foram alterados. Nenhum endpoint mutavel, usuario, perfil, permissao, vinculo, role ou GRANT foi criado nesta validacao. O uso previsto e servir a futura tela de atribuicao por checkboxes; proximos passos continuam sendo validacao em homologacao e planejamento separado de bloqueio/desbloqueio ou reset de senha.

Endpoints administrativos de bloqueio/desbloqueio implementados: `POST /api/internal/admin/users/{usuario_id}/block` e `POST /api/internal/admin/users/{usuario_id}/unblock` foram criados sob `GEOPORTAL_INTERNAL_ROUTES_ENABLED`, com sessao autenticada, `require_permission("admin.usuarios.bloquear")` e header `X-Geoportal-Internal-Request: 1`. O bloqueio persiste em `mod_auth.usuarios.bloqueado_ate` com timestamp futuro e revoga sessoes ativas do usuario em `mod_auth.sessoes` preenchendo `revogado_em = now()`, sem `DELETE`. O desbloqueio limpa `bloqueado_ate` e nao cria sessao automaticamente. Ambos retornam envelope `usuario` sanitizado com `id`, `login`, `nome`, `email`, `ativo`, `bloqueado` booleano e `criado_em`, sem retornar `bloqueado_ate`, senha, `senha_hash`, token, cookie, `session_secret`, `DATABASE_URL`, SQL, role, GRANT, sessao ou auditoria. A implementacao nao altera senha, perfis ou permissoes, nao cria endpoint de reset, nao altera schema, nao cria migration, nao altera producao, NSSM, `.env`, frontend ou tela. Proxima etapa: validar em homologacao com `teste.criacao`, confirmando login bloqueado, sessoes revogadas e desbloqueio permitindo novo login com senha valida.
...
Validacao operacional de bloqueio/desbloqueio em homologacao: o commit `88ff004` Adiciona bloqueio interno de usuarios foi aplicado no servidor e validado com pytest completo: 462 passed. A validacao ocorreu em processo isolado de homologacao com `teste.criacao` (`id=8`). Matriz de privilegios confirmada: `usuarios_select=t`, `usuarios_insert=t`, `usuarios_update=t`, `usuarios_delete=f`; `sessoes_select=t`, `sessoes_insert=t`, `sessoes_update=t`, `sessoes_delete=f`. Nenhum novo GRANT foi necessario. Cenarios validados: 401 sem sessao; 403 sem header mutavel `X-Geoportal-Internal-Request: 1`; 200 bloqueio com `bloqueado=true`; 200 bloqueio idempotente; 401 sessao ativa revogada em `mod_auth.sessoes` com `revogado_em = now()`; 401 novo login bloqueado; 404 usuario inexistente; 200 desbloqueio com `bloqueado=false`; 200 desbloqueio idempotente; 200 login apos desbloqueio com sessao restaurada. Resposta retornou apenas `id`, `login`, `nome`, `email`, `ativo`, `bloqueado`, `criado_em`. Nao retornou `bloqueado_ate`, `senha_hash`, `token_hash`, token, cookie, `session_secret`, `DATABASE_URL`, SQL, role, GRANT, sessao, auditoria ou segredo. Variaveis temporarias foram limpas. Producao, NSSM, `.env` versionado e frontend nao foram alterados.
...
Resposta 200 planejada: usuario sanitizado com `id`, `login`, `nome`, `email`, `ativo`, `bloqueado`, `criado_em`, sem expor `senha`, `nova_senha`, `senha_hash`, `bloqueado_ate`, `atualizado_em`, `ultimo_login_em`, token, `token_hash`, cookie, `session_secret`, `DATABASE_URL`, SQL, role, GRANT, sessao, auditoria ou segredo.

Comportamentos esperados: 401 sem sessao; 403 sem permissao/header; 404 usuario inexistente; 422 para payload invalido ou senha fraca.

Nesta etapa apenas documental: nenhuma implementacao, migration, schema, usuario real, perfil, permissao, vinculo, role, GRANT, producao, NSSM, `.env` ou frontend foram alterados.

Proximos passos: implementar endpoint com Codex High; validar em homologacao (confirmar que senha antiga falha 401, senha nova funciona 200, sessoes antigas sao revogadas 401, usuario bloqueado continua bloqueado); depois planejar o primeiro endpoint interno de negocio do modulo Iluminacao.
...
- `GET /api/internal/admin/profiles`
- `GET /api/internal/admin/users`
- `POST /api/internal/admin/users`
- `GET /api/internal/admin/users/{usuario_id}`
- `POST /api/internal/admin/users/{usuario_id}/profiles`
