...
def test_bootstrap_is_idempotent_when_records_already_exist() -> None:
    permission_rows = [{"id": index, "ativo": True} for index in range(101, 111)]
    profile_permission_rows = [{"exists": 1} for _ in range(10)]
    ...
    assert response.permissao_ids == tuple(range(101, 111))
...
def test_bootstrap_creates_missing_records_without_sensitive_output_or_delete() -> None:
    permission_insert_rows = [{"id": index} for index in range(101, 111)]
    ...
    rows.extend([None for _ in range(10)])
    rows.append(None)
    engine = FakeEngine(rows)
...
def test_bootstrap_rejects_inactive_profile_without_reactivation() -> None:
    rows: list[dict[str, Any] | None] = [
        {"id": 7},
        *({"id": index, "ativo": True} for index in range(101, 111)),
        {"id": 20, "ativo": False},
    ]
    engine = FakeEngine(rows)
