...
Bloqueio/desbloqueio implementados no backend: `POST /api/internal/admin/users/{usuario_id}/block` e `POST /api/internal/admin/users/{usuario_id}/unblock` usam apenas colunas existentes. O bloqueio atualiza `mod_auth.usuarios.bloqueado_ate` para timestamp futuro e revoga sessoes ativas em `mod_auth.sessoes.revogado_em`, sem `DELETE`. O desbloqueio limpa `bloqueado_ate` e nao cria sessao. As respostas expoem apenas o booleano derivado `bloqueado`, nunca `bloqueado_ate`. Para validacao operacional futura, qualquer permissao adicional deve ser planejada com menor privilegio: `UPDATE` restrito em `mod_auth.usuarios` para bloqueio/desbloqueio e `UPDATE` em `mod_auth.sessoes` para revogacao logica, sem ampliar acesso a senha, perfis, permissoes ou auditoria.
